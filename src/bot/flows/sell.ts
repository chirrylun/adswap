  ...
  switch (type) {
    case 'google_ad_account':
      return [
        ...
        { ...COUNTRY_QUESTION, prompt: COUNTRY_QUESTION.prompt.replace('Final Step', 'Step 6 of 6') },
      ];

    case 'facebook_ad_account':
      return [
        { step: 'sell_q_meta_age',   prompt: `*Step 1 of 6 — Account Age* 📅\n\nHow old is this Facebook/Meta Ads account?\n\nExamples: _6 months_, _2 years_\n\nType your answer:` },
        { step: 'sell_q_meta_limit', prompt: `*Step 2 of 6 — Spend Limit* 💳\n\nWhat is the current daily or total spend limit?\n\nExamples: _$50/day_, _$500 total_, _No limit_\n\nType your answer:` },
        {
          step:    'sell_q_meta_bm',
    ...
    case 'adsense_site':
      return [
        { step: 'sell_q_ads_age',      prompt: `*Step 1 of 6 — Account Age* 📅\n\nHow old is this AdSense account?\n\nExamples: _1 year_, _3 years_\n\nType your answer:` },
        {
    ...
    case 'play_console':
      return [
        ...
        { ...COUNTRY_QUESTION, prompt: COUNTRY_QUESTION.prompt.replace('Final Step', 'Step 5 of 5') },
      ];

    case 'gift_card':
      return [
        { step: 'sell_q_gc_brand',    prompt: `*Step 1 of 3 — Card Brand* 🎁\n\nWhat brand is this gift card?\n\nExamples: _Amazon_, _iTunes/Apple_, _Steam_, _Google Play_, _Visa_\n\nType your answer:` },
        { step: 'sell_q_gc_value',    prompt: `*Step 2 of 3 — Card Value* 💵\n\nWhat is the face value of this card?\n\nExamples: _$50_, _$100_, _£25_\n\nType your answer:` },
        { step: 'sell_q_gc_currency', prompt: `*Step 3 of 3 — Card Region* 🌍\n\nWhat region/country is this card valid for?\n\nExamples: _USA_, _UK_, _Global_\n\nType your answer:` },
      ];
...
function buildDescription(type: string, data: Record<string, any>): string {
  ...
  const country = data.sell_q_country ? ` | Country: ${data.sell_q_country}` : '';
...
function buildListingFields(type: string, data: Record<string, any>): Record<string, any> {
  const country = type !== 'gift_card' && data.sell_q_country
  ...
  switch (type) {
    ...
    case 'adsense_site':
      return {
        ...
        adsensePaymentStatus:   data.sell_q_ads_payment === 'ADS_PAY_YES'
          ? 'received'
          : data.sell_q_ads_payment === 'ADS_PAY_THRESH'
          ? 'threshold'
          : 'none',
        adsenseMonthlyEarnings: data.sell_q_ads_earnings,
        adsenseSiteUrl:         data.sell_q_ads_url?.toUpperCase() === 'NONE'
          ? undefined
          : data.sell_q_ads_url,
...
export async function handleSell(
  ...
  if (text === 'SELL') {
    ...
    return sendMessage(phone,
      ...
      `💡 *Listing is free.* AdSwap adds a small service fee to the buyer's price — you receive your full asking amount.\n\n` +
  ...
  if (step === 'sell_type') {
    ...
    if (!typeKey) return sendMessage(phone, '❌ Please reply with a number from 1 to 8.');
    ...
    return sendMessage(phone,
      ...
      `Example: *75000*\n\n` +
      `Minimum: ₦1,000\n\n` +
      `💡 You will receive this *full amount*. AdSwap's service fee is added on top for the buyer.\n\n` +
      `Type *CANCEL* to exit.`,
    );
  }
