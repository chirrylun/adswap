...
router.post(
  ...
  async (req: AdminRequest, res: Response) => {
    ...
    await sendMessage(
      ...
      approvalSellerMessage(listing.listingId),
    ).catch(() => {});
...
router.post(
  ...
  async (req: AdminRequest, res: Response) => {
    ...
    if (!txn)
      return res
        .status(404)
        .json({ error: "Transaction not found or not in pending status" });
...
router.post(
  ...
  async (req: AdminRequest, res: Response) => {
    ...
    if (!txn)
      return res
        .status(404)
        .json({ error: "Transaction not found or not cancelled" });
...
router.post(
  ...
  async (req: AdminRequest, res: Response) => {
    ...
    if (!["buyer", "seller"].includes(decision)) {
      return res
        .status(400)
        .json({ error: "Decision must be buyer or seller" });
    }
...
router.get("/users", adminAuth, async (req: AdminRequest, res: Response) => {
  ...
  const [users, total] = await Promise.all([
    User.find(filter)
      .select(
        "phone name isBanned banReason sellerRating totalSales totalPurchases lastActiveAt joinedAt notifications isVerified",
      )
      .sort({ lastActiveAt: -1 })
      .skip((+page - 1) * +limit)
      .limit(+limit),
