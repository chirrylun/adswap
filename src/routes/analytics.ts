...
router.get('/open-rate', adminAuth, async (req: AdminRequest, res: Response) => {
  const d        = since(req.query.days ?? 30);
  ...
  const [sent, delivered, read, failed, avgTtRead] = await Promise.all([
    ...
    MessageLog.countDocuments({ ...baseFilter, status: { $in: ['delivered', 'read'] } }),
  ...
  if (!listingId && (!category || category === 'new_listing')) {
    topListings = await MessageLog.aggregate([
      ...
      { $match: { sent: { $gte: 10 } } }, // minimum sample size
      {
        $addFields: {
          openRate: {
            $multiply: [{ $divide: ['$read', '$sent'] }, 100],
  ...
  const openRate     = sent > 0 ? ((read     / sent) * 100).toFixed(1) : '0.0';
  const avgTtReadSec = avgTtRead[0]?.avg ?? null;

  res.json({
    since: d,
    filter: { category: listingId ? undefined : (category ?? 'new_listing'), listingId },
    summary: {
      sent,
      delivered,
      read,
      ...
      openRate:     `${openRate}%`,
      avgTtReadMin: avgTtReadSec !== null
        ? Math.round(avgTtReadSec / 60)
        : null,
    },
